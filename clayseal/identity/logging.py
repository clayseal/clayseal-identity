...
Provides a configured ``clayseal`` logger. Format is auto-detected from the
...
def _detect_format() -> str:
    ...
    Honour an explicit ``CLAYSEAL_LOG_FORMAT`` first; otherwise emit JSON when
    ...
    """
    explicit = os.getenv("CLAYSEAL_LOG_FORMAT")
    if explicit:
...
class _JsonFormatter(logging.Formatter):
    def format(self, record: logging.LogRecord) -> str:
        ...
        extra = getattr(record, "clayseal", None)
    ...
    *,
    ...
    if fmt == "json":
        ...
        handler.setFormatter(
            logging.Formatter("%(asctime)s %(levelname)s [clayseal] %(message)s")
        )
    return logger
