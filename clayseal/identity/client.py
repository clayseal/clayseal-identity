"""``ClaySeal`` - the tenant-level entry point.
...
class ClaySeal:
    def __init__(
        ...
        *,
        ...
        api_key = api_key or os.getenv("CLAYSEAL_API_KEY")
        base_url = base_url or os.getenv("CLAYSEAL_BASE_URL") or DEFAULT_BASE_URL
        self.base_url = base_url

        mtls_cert = mtls_cert or os.getenv("CLAYSEAL_MTLS_CERT_FILE")
        mtls_key = mtls_key or os.getenv("CLAYSEAL_MTLS_KEY_FILE")
        mtls_ca = mtls_ca or os.getenv("CLAYSEAL_MTLS_CA_FILE")
    ...
    def identify(
        ...
        *,
        ...
        For local demos/tests, construct ``ClaySeal(..., dev_attestation=True)``
        or set ``CLAYSEAL_DEV_ATTESTOR=1``. That mode self-registers a throwaway
        node trust anchor + registration entry and is allowed only against a
        localhost backend unless ``CLAYSEAL_ALLOW_REMOTE_DEV_ATTESTOR=1`` is set.
            ...
            if not self._dev_attestation:
                raise ClaySealError(
                    ...
                    suggestion=(
                        ...
                        "ClaySeal(..., dev_attestation=True) for localhost-only demos/tests."
                    ),
                )
            if not _is_local_base_url(self.base_url) and not _env_truthy(UNSAFE_DEV_ATTESTOR_ENV):
                raise ClaySealError(
                    ...
                    suggestion=(
                        ...
                        "test environment only, set CLAYSEAL_ALLOW_REMOTE_DEV_ATTESTOR=1."
                    ),
                )
