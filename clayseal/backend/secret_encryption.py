...
PROVIDER_ENV = "CLAYSEAL_SECRET_ENCRYPTION_PROVIDER"
LOCAL_MASTER_KEY_ENV = "CLAYSEAL_SIGNING_KEY_ENCRYPTION_KEY"
AWS_KMS_KEY_ID_ENV = "CLAYSEAL_AWS_KMS_KEY_ID"
GCP_KMS_KEY_NAME_ENV = "CLAYSEAL_GCP_KMS_KEY_NAME"
REQUIRE_ENCRYPTION_ENV = "CLAYSEAL_REQUIRE_SECRET_ENCRYPTION"
...
def _is_production() -> bool:
    ...
    # correct under tests that toggle CLAYSEAL_ENV via monkeypatch.
    return os.getenv("CLAYSEAL_ENV", "development").strip().lower() == "production"
...
def _cached_boto3_kms_client():
    ...
    try:
        ...
        raise RuntimeError(
            "boto3 is required when CLAYSEAL_SECRET_ENCRYPTION_PROVIDER=aws_kms"
        ) from exc
...
def _cached_gcp_kms_client():
    ...
    try:
        ...
        raise RuntimeError(
            "google-cloud-kms is required when CLAYSEAL_SECRET_ENCRYPTION_PROVIDER=gcp_kms"
        ) from exc
