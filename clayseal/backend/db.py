...
(Postgres in production) is configured via ``CLAYSEAL_DATABASE_URL`` and gets a
...
def validate_database_config(settings: Settings | None = None) -> None:
    ...
    if settings.is_production and is_sqlite_url(normalize_database_url(settings.database_url)):
        raise RuntimeError(
            "CLAYSEAL_ENV=production requires a non-SQLite CLAYSEAL_DATABASE_URL "
...
def init_db() -> None:
    ...
    When ``CLAYSEAL_MANAGE_SCHEMA=alembic`` this is a no-op: migrations are
