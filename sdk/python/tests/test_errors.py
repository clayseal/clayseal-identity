...
from clayseal.identity.errors import from_envelope
...
def test_identify_requires_explicit_attestation_or_dev_mode(api_key, base_url):
    client = ClaySeal(api_key=api_key, base_url=base_url)
    with pytest.raises(ClaySealError) as exc:
        client.identify(agent_type="x", owner="y", scopes=[])
...
def test_dev_attestation_is_localhost_only(api_key):
    client = ClaySeal(
        api_key=api_key,
        base_url="https://clayseal.example.test",
...
def test_from_envelope_unknown_code_falls_back_to_base():
    ...
    assert isinstance(err, ClaySealError)
