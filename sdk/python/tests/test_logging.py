...
from clayseal.identity import logging as sdk_logging


def test_detect_format_honors_explicit_env(monkeypatch):
    monkeypatch.setenv("CLAYSEAL_LOG_FORMAT", "TEXT")

    assert sdk_logging._detect_format() == "text"


def test_detect_format_uses_structured_environment(monkeypatch):
    monkeypatch.delenv("CLAYSEAL_LOG_FORMAT", raising=False)
    monkeypatch.setenv("DD_SERVICE", "clayseal-tests")
...
def test_get_logger_formats_json_extra_and_reconfigures_text():
    logger = logging.getLogger("clayseal")
    ...
    record = configured.makeRecord(
        ...
        (),
        None,
        extra={"clayseal": {"agent_id": "agent-1", "customer_id": "cust-1"}},
    )
