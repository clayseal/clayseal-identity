...
def require_admin(
    ...
    When ``AGENTAUTH_ADMIN_API_KEY`` is configured, the caller must present a
    ...
    if not admin_key:
        if settings.is_production:
            raise InvalidAPIKeyError(
                ...
                suggestion=(
                    "Set AGENTAUTH_ADMIN_API_KEY on the server to enable admin-gated "
    ...
    if not x_admin_key or not hmac.compare_digest(x_admin_key, admin_key):
        raise InvalidAPIKeyError(
            ...
            suggestion="Send the AGENTAUTH_ADMIN_API_KEY value in the 'X-Admin-Key' header.",
        )
