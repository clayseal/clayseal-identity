...
from clayseal.backend.config import Settings, get_settings
from clayseal.backend.db import (
    create_db_engine,
    is_sqlite_url,
...
def test_postgres_engine_gets_tuned_pool(monkeypatch):
    monkeypatch.setenv("CLAYSEAL_DB_POOL_SIZE", "7")
    monkeypatch.setenv("CLAYSEAL_DB_MAX_OVERFLOW", "3")
    monkeypatch.setenv("CLAYSEAL_DB_POOL_RECYCLE", "1234")
    monkeypatch.setenv("CLAYSEAL_DB_POOL_TIMEOUT", "25")
    monkeypatch.setenv("CLAYSEAL_DB_POOL_PRE_PING", "1")
    settings = Settings()
...
def test_production_rejects_sqlite(monkeypatch):
    ...
    monkeypatch.setenv("CLAYSEAL_DATABASE_URL", "sqlite:///./agents.db")
...
def test_production_accepts_postgres(monkeypatch):
    monkeypatch.setenv("CLAYSEAL_ENV", "production")
    monkeypatch.setenv("CLAYSEAL_DATABASE_URL", "postgresql://u:p@h/db")
